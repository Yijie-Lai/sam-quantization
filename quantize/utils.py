...
def load_quantized_model(model_path, wbits, group_size):
    ...
    # ===== Step 2: replace Linear -> QuantLinear =====
    for i in tqdm(range(len(layers)), desc="Replacing Linear with QuantLinear"):
        layer = layers[i]
        named_linears = get_named_linears(layer, nn.Linear)
    ...
    print("Loading quantized weights (fake quant inplace)...")
