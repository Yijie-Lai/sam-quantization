...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    def load_data(dataset_name):
        if dataset_name == 'alpaca':
            ...
            local_dataset = args.redpajama_data_path
            if local_dataset:
                if not os.path.isfile(local_dataset):
                    raise FileNotFoundError(
                        f"RedPajama JSONL does not exist: {local_dataset}"
                    )
                print(f"loading local RedPajama data from {local_dataset}")
                dataset = load_dataset(
                    "json", data_files={"train": local_dataset}
                )
            else:
                dataset = load_dataset(
    ...
    if args.dataset in ['c4', 'redpajama']:
        ...
        cache_dir = './cache'
        general_ratio = args.general_data_ratio
        if not 0.0 <= general_ratio < 1.0:
            raise ValueError("general_data_ratio must satisfy 0.0 <= ratio < 1.0")
        ratio_tag = f'{general_ratio:.4f}'.replace('.', 'p')
        cache_dataloader = f'{cache_dir}/dataloader_{args.model_family}_{args.dataset}_{ratio_tag}_{args.pt_context_len}.cache'
        if os.path.exists(cache_dataloader):
            dataset = torch.load(cache_dataloader)
            print(f"load dataset from {cache_dataloader}")
        else:
            ...
            # Select the small general-domain portion before tokenization.
            general_size = int(
                len(deita_dataset) * general_ratio / max(1.0 - general_ratio, 1e-8)
            )
            dataset = load_data('redpajama')
            raw_candidate_size = min(general_size * 4, len(dataset['train']))
            dataset['train'] = dataset['train'].shuffle(seed=0).select(
                range(raw_candidate_size)
            )
            redpajama_dataset = format_dataset(dataset, 'pt')

            general_size = min(general_size, len(redpajama_dataset['train']))
            mixed_parts = [deita_dataset]
            if general_size > 0:
                general_dataset = redpajama_dataset['train'].select(range(general_size))
                mixed_parts.append(general_dataset)
            train_dataset = concatenate_datasets(mixed_parts).shuffle(seed=0)
            print(
                f'mixed dataset: deita={len(deita_dataset)}, '
                f'general={general_size}, general_ratio={general_ratio:.2%}'
            )
            dataset = {
                "train":train_dataset,
